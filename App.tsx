...
const App: React.FC = () => {
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const initialBatch = await generateQuizBatch(
        content,
        [],
        apiKey,
        isImage,
      ...
      setState((prev) => ({
        ...prev,
        isLoading: false,
        error: err.message || "A kinyerés sikertelen.",
      }));
    }
  };
  ...
  const loadMoreQuestions = async (isAuto = false) => {
    ...
    try {
      ...
      const nextBatch = await generateQuizBatch(
        activeFile.content,
        existingTitles,
        apiKey,
        isImage,
  ...
  const score = useMemo(() => {
    ...
    state.checkedQuestions.forEach((idx) => {
      ...
      if (
        q &&
        user.length === q.correct_answers.length &&
        user.every((a) => q.correct_answers.includes(a))
      ) {
        count++;
      }
    });
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-12 px-4">
      <header className="max-w-3xl w-full mb-12 text-center">
        ...
        <p className="text-slate-600 font-medium mb-6">
          Kérdések kinyerése a dokumentumból (Max {MAX_QUESTIONS} kérdés).
        </p>
        ...
        <div className="max-w-md mx-auto relative group">
          ...
          <p className="text-[11px] text-slate-500 mt-2 font-medium">
            Ingyenes kulcs igénylése itt:{" "}
            <a
              href="https://aistudio.google.com/app/apikey"
              target="_blank"
              rel="noreferrer"
              className="text-slate-900 underline decoration-slate-400 hover:text-slate-700 hover:decoration-slate-700 transition-all"
            >
              Google AI Studio
            </a>
            . ("Create API Key a jobb felső sarokban, majd "Create New Project"
            és azután "Create key" azután kimásolod a másolás gombbal. Csak
            Gemini API felel meg".)
          </p>
        </div>
      </header>
          ...
          <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-4 shadow-xl border border-slate-200 flex flex-col gap-2 relative overflow-hidden">
            {isFetchingMore && (
              <div className="absolute top-0 left-0 w-full h-1 bg-slate-900/10">
                <div
                  className="h-full bg-slate-900 animate-[loading_1.5s_infinite_linear]"
                  style={{ width: "30%" }}
                ></div>
              </div>
            )}
            ...
            <div className="flex justify-between items-center px-1">
              ...
              <span className="text-sm font-bold text-slate-800">
                Helyes: {score} | {state.checkedQuestions.size} /{" "}
        ...
        {!state.isLoading && state.questions.length === 0 && (
          <div className="bg-white border-2 border-dashed border-slate-200 rounded-3xl p-12 flex flex-col items-center text-center transition-all hover:border-slate-400 hover:bg-slate-50/50 group">
            ...
            <h2 className="text-2xl font-semibold mb-2">
              Forrásanyag feltöltése
            </h2>
            <p className="text-slate-500 mb-8 max-w-sm">
              Először 15 véletlenszerű kérdést nyerünk ki. A további kérdések
              automatikusan töltődnek be, amíg van elérhető tartalom (max{" "}
              {MAX_QUESTIONS}).
            </p>
        ...
        {state.isLoading && (
          <div className="bg-white rounded-3xl shadow-sm p-12 flex flex-col items-center text-center">
            ...
            <p className="text-slate-500">
              A dokumentum feldolgozása folyamatban.
            </p>
          </div>
        )}
              ...
              {isFetchingMore ? (
                ...
                >
                  ...
                  <span className="text-xs font-medium text-slate-400">
                    Még {MAX_QUESTIONS - state.questions.length} kérdés nyerhető
                    ki
                  </span>
                </button>
              ) : (
                <div className="text-center py-6 text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px]">
                  Nem található több kérdés vagy elértük a {MAX_QUESTIONS}-es
                  limitet
                </div>
              )}
            </div>

            {state.checkedQuestions.size === state.questions.length &&
              !isFetchingMore &&
              hasReachedEnd && (
                <div className="bg-slate-900 rounded-3xl p-10 text-center shadow-2xl shadow-slate-400 text-white mt-12 animate-in zoom-in duration-500">
                  ...
                  <p className="text-slate-400 mb-6 text-lg">
                    Végeredmény: {score} a(z) {state.questions.length}-ból
                  </p>
      ...
      <footer className="mt-12 text-slate-400 text-xs font-bold uppercase tracking-widest opacity-60">
        Folyamatos kinyerés • Max {MAX_QUESTIONS} kérdés • Duplikáció szűrés •
        Gemini 3 Pro
      </footer>
    </div>
  );
...
const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  return (
    <div
      className={`bg-white rounded-[2.5rem] shadow-sm p-8 transition-all duration-500 border-2 ${
        isChecked
          ? isCorrect
            ? "border-green-200 ring-8 ring-green-50/50 shadow-green-100/50"
            : "border-red-200 ring-8 ring-red-50/50 shadow-red-100/50"
    ...
    >
      <div className="flex items-start gap-4 mb-8">
        <span
          className={`flex items-center justify-center w-10 h-10 rounded-2xl font-black text-sm shrink-0 transition-all ${
            isChecked
              ? isCorrect
      ...
      <div className="grid grid-cols-1 gap-3">
        {question.options.map((option, i) => {
          ...
          let optionStyles =
            "flex items-center gap-4 p-5 rounded-3xl border-2 transition-all cursor-pointer relative group ";
          ...
          if (!isChecked) {
            ...
            if (isCorrectOption) {
              ...
              optionStyles +=
                "bg-red-50 border-red-500 text-red-800 font-bold shadow-sm";
            } else {
              optionStyles +=
                ...
                {isUserChoice &&
                  (isChecked && !isCorrectOption ? (
                    ...
                    <CheckSquare
                      size={14}
                      className={
                        isChecked ? "text-green-600" : "text-slate-800"
                      }
                    />
                  ))}
                {!isUserChoice && isChecked && isCorrectOption && (
                  <CheckSquare size={14} className="text-green-600/50" />
                )}
              </span>
      ...
      {!isChecked ? (
        ...
        <div
          className={`mt-8 p-6 rounded-[2.5rem] animate-in fade-in zoom-in slide-in-from-top-4 duration-500 ${
            isCorrect
              ? "bg-green-50 text-green-900 border-l-8 border-green-500"
        ...
        >
          <div className="flex items-center gap-3 mb-3 font-black text-xs uppercase tracking-[0.1em]">
            ...
            {isCorrect
              ? "Kiváló"
              : `Megoldás: ${question.correct_answers.join(", ")}`}
          </div>
          <div className="text-sm leading-relaxed space-y-4">
            <p className="bg-white/40 p-4 rounded-2xl italic font-medium text-slate-600">
              "Eredetileg a(z) {question.original_index}. kérdés a
              dokumentumban."
            </p>
            <div className="pl-1">
              <span className="font-black text-[10px] uppercase text-slate-400 block mb-2 tracking-widest">
                Miért ez a válasz?
              </span>
